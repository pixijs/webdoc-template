...
function runLinkTag(input) {
    ...
    while (linkMatch) {
        ...
        if (isValidUrl(link)) {
            ...
            replaced = (0, linker_1.linkTo)(link, linkText);
        }
...
function runTagResolver(input) {
    input = runCodeTag(input);
    input = runLinkTag(input);
    return input;
}
exports.runTagResolver = runTagResolver;
